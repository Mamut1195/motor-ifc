...
class LegacyAdapterV1:
    def get_version(self,params): return {"version":VERSION}
    def inspect_ifc(self,params): return inspect_ifc(_required(params,"path")).model_dump(mode="json")
    def extract_spatial_tree(self,params):
        result=inspect_ifc(_required(params,"path")); return {"success":result.success,"spatial_tree":result.spatial_tree,"diagnostics":[d.model_dump(mode="json") for d in result.diagnostics]}
    def validate_basic(self,params): return validate_ifc(_required(params,"path")).model_dump(mode="json")
...
def dispatch(method:str,params:Any,job_root:str|None=None)->Any:
    ...
    if method=="model.inspect.v1": return inspect_ifc(_required(params,"path")).model_dump(mode="json")
    if method=="reader.extract.v1": return extract_ifc(_reader_path(params,job_root)).model_dump(mode="json")
    if method=="model.validate.v1": return validate_ifc(_required(params,"path"),ValidationPolicy.model_validate(params.get("policy",{}))).model_dump(mode="json")
    ...
    if method in {"get_version","inspect_ifc","extract_spatial_tree","validate_basic"}: return getattr(_LEGACY,method)(params)
